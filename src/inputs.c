...
/*
 * File: inputs.c
 * Purpose: Input-driven setup helpers and wall validity checks.
 */

/* Mark the cell and its neighbors as occupied in the placement mask. */
...
int ControllingWalls(int WallCount, int n, int m, int walls[][2], char WallsState[], int isWall[][15][2])
{
    ...
    for (int i = 0; i < WallCount; i++)
    {
        ...
        int state, X, Y;
        do{
            state = rand() % 2; // Horizontal or vertical
            if (state == 0)
            {
                X = rand() % (n - 1);
                Y = rand() % m;
