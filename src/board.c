...
/*
 * File: board.c
 * Purpose: Board rendering, wall handling, and game state persistence helpers.
 * Notes: Contains both draw routines and save/load logic.
 */
...
/* Draw the board grid lines with padding. */
...
/* Render players or hunters with selection and animation effects. */
void Showingpieces(Texture2D piece, int piecesNo, float pieces[][2], int cellWidth, int cellHeight, int playerMoved[], int state, float timer, float *transparency, char *s, int selected, int *PickedHunter, int *MovePlayerBonus)
{
    ...
    // state = 1 -> for huters
    float x, y, pX, pY;
    float scale, radius, centerX, centerY;
    if (*transparency >= 1.0)
        *s = 'D'; // Decrease transparency
    if (*transparency <= 0.3)
        *s = 'I'; // Increase transparency
    if (*s == 'I')
        *transparency += GetFrameTime() / 3;
    else
        *transparency -= GetFrameTime() / 3;
    for (int i = 0; i < piecesNo; i++)
    {
        ...
        if (state == 0 && playerMoved[i] == 0)
        {
            ...
            {
                if (timer >= 0.6 || *MovePlayerBonus != -1)
                    DrawTextureEx(piece, (Vector2){pX, pY}, 0, scale, Fade(WHITE, *transparency));
                else
                    DrawTextureEx(piece, (Vector2){pX, pY}, 0, scale, WHITE);
            }
            else
...
/* Build rectangles for hover/placement of temporary walls. */
...
/* Place a temporary wall at the mouse position if valid. */
...
/* Render all walls that are active or still visible this turn. */
...
/* Show a preview wall under the mouse cursor. */
...
{
    FILE *game = fopen("../data/game.dat", "wb");
...
/* Load or prompt for initial board configuration and UI sizing. */
...
/* Load a saved game or generate a new board, entities, and bonuses. */
...
{
    *pieceRed = LoadTexture("../assets/pieces/redPieces.png");
    SetTextureFilter((*pieceRed), TEXTURE_FILTER_TRILINEAR);
    *pieceBlue = LoadTexture("../assets/pieces/bluePieces.png");
    SetTextureFilter((*pieceBlue), TEXTURE_FILTER_TRILINEAR);
    *box = LoadTexture("../assets/bonus/BonusBox.png");
    SetTextureFilter((*box), TEXTURE_FILTER_TRILINEAR);
    *f = LoadFontEx("../assets/fonts/LuckiestGuy-Regular.ttf", maxsize, 0, 0);
