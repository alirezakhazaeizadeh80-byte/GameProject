...
/*
 * File: pathfinding.c
 * Purpose: A* pathfinding utilities over the grid with wall constraints.
 */
...
int isUnBlocked(int rows, int cols, Pair ori, Pair dest, int isWall[][15][2])
{
    ...
    {
        if (isWall[ori.row - 1][ori.col][0] == 1)
            return 0;
        else
            return 1;
    }
    ...
    {
        if (isWall[ori.row][ori.col - 1][1] == 1)
            return 0;
        else
            return 1;
    }
    ...
    while (opencount)
    {
        ...
        if (isValid(rows, cols, i - 1, j))
        {
            ...
            else if (closedlist[i - 1][j] == 0 && isUnBlocked(rows, cols, (Pair){i, j}, (Pair){i - 1, j}, isWall) == 1 && isBonus[i - 1][j] == 0)
            {
                ...
                {
                    if (openlist[m].p.row == i - 1 && openlist[m].p.col == j)
                        sw = 0;
                }
        ...
        if (isValid(rows, cols, i, j + 1))
        {
            ...
            else if (closedlist[i][j + 1] == 0 && isUnBlocked(rows, cols, (Pair){i, j}, (Pair){i, j + 1}, isWall) == 1 && isBonus[i][j + 1] == 0)
            {
                ...
                for (int m = 0; m < opencount; m++)
                {
                    if (openlist[m].p.row == i && openlist[m].p.col == j + 1)
                        sw = 0;
                }
